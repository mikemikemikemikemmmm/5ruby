...
export default function CarouselCotainer(props) {
  const { formFeedSecond = 2, children, hasArrow, itemKeyClass } = props;
  ...
  return (
    <div className="carouselCotainer position-r h-100per">
      {hasArrow ? (
        <>
          <button
      ...
      >
        {children.map(item => (
          <div
            ...
            className={`carouselItem d-inline-block vw-100 ${itemKeyClass}`}
          >
            {item}
          </div>
        ))}
